...
def long_moving_avg_greater_than_short_moving_avg(context, data, equity):
    # moving averages- indicators
    short_mavg = data.history(equity, 'price', bar_count=100, frequency="1d").mean()
    long_mavg = data.history(equity, 'price', bar_count=300, frequency="1d").mean()
    return short_mavg < long_mavg

def price_is_trending_down(context, data, equity):
