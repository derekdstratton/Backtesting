...
def initialize(context):
    ...
    # context.end_session
    #todo make a function that filters out stocks that undergoes splits in the range
    ...
    equities = ['AAPL', 'MSFT']
    #todo try filling this with a bunch of data from a file to watch
...
def handle_data(context, data):
    ...
    #todo theroetically its smartest to analyze all stocks (in a for loop)
    #todo and after deciding what's best, then to put in orders
...
def analyze(context, perf):
    ...
    print("Starting: %f" % perf.portfolio_value[0])
    print("Ending at: %f:" % perf.portfolio_value[perf.portfolio_value.size - 1])

    if perf.portfolio_value[0] == 0:
        percent_growth = -1
    else:
        percent_growth = (perf.portfolio_value[perf.portfolio_value.size - 1] / perf.portfolio_value[0]) - 1
    print(percent_growth)
    data = open("simulatordata.csv", 'a')
    data.write(str(percent_growth))
    data.close()
