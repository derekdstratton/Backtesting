...
def handle_data(context, data):
    ...
    #i is the number of orders opened in a single, daily run-through
    i = 0
    #the amount of cash trackced in a single daily run-through must also be manually tracked
    # todo important note: context.portfolio is updated BETWEEN EACH DAY.
    # buying, look at all equities that i'm viewing
    for equity in context.equities:
        #  print(len(context.portfolio.positions))
        if len(context.portfolio.positions) < 5 and i < 5:
